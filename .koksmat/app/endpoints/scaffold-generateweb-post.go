...
func ScaffoldGeneratewebPost() usecase.Interactor {
	type Request struct {
	...
	u := usecase.NewInteractor(func(ctx context.Context, input Request, output *string) error {
		...
		_, err := execution.ExecutePowerShell("john", "*", "365admin-publish", "scaffold", "11-generateweb.ps1", "", "-kitchenname", input.Kitchenname)
		if err != nil {
			return err
		}
